    ...
    for(row=tn; row<tn+ip.Vpixels/NumThreads; row++){
        ...
        while(col<hp3){
			...
			if((NewCol>=0) && (NewRow>=0) && (NewCol<ip.Hpixels) && (NewRow<ip.Vpixels)){
				...
				memcpy((void*)(*(CopyImage + NewRow)+ NewCol), (void*) (Buffer+col), (size_t) 3);
...
void *Rotate9(void* ThCtr)
{
    // long tn;            		     // My thread number (ID) is stored here
//    int row,col,h,v,c;
    int row,col,h,v,c, hp3, Last, ctr;
	double cc, ss, k1, k2;
	int NewRow,NewCol;
	double X, Y, newX, newY, ScaleFactor;
	double Diagonal, H, V;
    ...
    do{
		...
		// printf("\n the row in processing is: %d", row);
	...
	//      while(col<ip.Hpixels*3){
			memcpy((void *) Buffer, (void *) TheImage[row], (size_t) ip.Hbytes);
	        for(col=0; col<ip.Hpixels*3; col=col+3){
	        	newX=cc-k1;
				newY=ss-k2;
				NewCol=((int) newX+h);
				NewRow=v-(int)newY;  
				...
				if((NewCol>=0) && (NewRow>=0) && (NewCol<ip.Hpixels) && (NewRow<ip.Vpixels)){
					NewCol*=3;
					// printf("%u ", (unsigned int) *((*TheImage + row*ip.Hbytes)+ col*3)); 
					// memcpy((void*)Buffer, (void*)(*(TheImage + row)+ col), (size_t) 3);
					memcpy((void*)(*(CopyImage + NewRow)+ NewCol), (void*) (Buffer+col), (size_t) 3);
					// CopyImage[NewRow][NewCol]   = TheImage[row][col];
					// CopyImage[NewRow][NewCol+1] = TheImage[row][col+1];
					// CopyImage[NewRow][NewCol+2] = TheImage[row][col+2];
...
int main(int argc, char** argv)
{
	...
	if(RotateFunc==Rotate9){
		...
		// printf("\n the thread id is: %d", i);
			ThErr = pthread_create(&ThHandle[j], &ThAttr, RotateFunc, (void *)&ThreadCtr[j]);
			if(ThErr != 0){
